...
namespace ft {
    ...
    public:
		...
		template< class T_BST, class Compare_BST, class value_alloc = std::allocator<T_BST>, class Node = ft::node<T_BST>, class node_alloc = std::allocator<ft::node<T_BST> > >
	    class BST {
	    	friend class map<key_type, mapped_type, key_compare, Alloc>;
	    protected:
			typedef T_BST value_type_bst;
			typedef typename ft::bst_iterator<value_type_bst, Compare> iterator_bst;
			typedef typename ft::const_bst_iterator<value_type_bst, Compare> const_iterator_bst;
			typedef Compare_BST key_compare;
			typedef size_t size_type;
			typedef typename value_type_bst::first_type key_type;
			typedef typename value_type_bst::second_type second_type;
		...
		protected:
			...
			iterator_bst begin() {
				if (_size == 0)
					return (iterator_bst(_end));
				Node *head = _root;
				while (head->_left)
					head = head->_left;
				return (iterator_bst(head));
			}
			...
			const_iterator_bst begin() const {
				if (_size == 0)
					return (const_iterator_bst(_end));
				Node *head = _root;
				while (head->_left)
					head = head->_left;
				return (const_iterator_bst(head));
			}

			const_iterator_bst end() const { 
				return (const_iterator_bst(_end));
			}
	    	...
	    	ft::pair<iterator_bst,bool> add(const T_BST& pair, bool hint = false, iterator_bst hint_it = NULL) {
	    		if (_size == 0) {
	    			++_size;
					_value_alloc.construct(_root->_value, pair);
					insert_case_1(_root);
	    			return (ft::pair<iterator_bst,bool>(iterator_bst(_root), true));
	    		}
	    		...
	    		while (next != NULL && next != _end) {
	    			...
	    			if (_comp(pair.first, next->_value->first))
	    				...
	    				return (ft::pair<iterator_bst,bool>(iterator_bst(next), false));
	    		}
				...
				return (ft::pair<iterator_bst,bool>(iterator_bst(new_node), true));
			}
			...
			iterator_bst lower_bound (const key_type& k) {
				Node *head = _root;
				iterator_bst candidate = this->end();
			...
			iterator_bst upper_bound (const key_type& k) {
				Node *head = _root;
				iterator_bst candidate = this->end();
			...
			iterator_bst find (const key_type& k) {
				Node *head = _root;
			...
			const_iterator_bst find (const key_type& k) const {
				Node *head = _root;
			...
			const_iterator_bst lower_bound (const key_type& k) const {
				Node *head = _root;
				const_iterator_bst candidate = this->end();
			...
			const_iterator_bst upper_bound (const key_type& k) const {
				Node *head = _root;
				const_iterator_bst candidate = this->end();
			...
			void erase(iterator_bst position) {
			...
			size_type erase (const key_type& k) {
				iterator_bst locate = this->find(k);
				if (locate == this->end()) {
					return (0);
				}
				this->erase(locate);
			...
			void erase (iterator_bst first, iterator_bst last) {
				if (first == last)
					return ;
				iterator_bst head = last;
		...
		typedef typename BST<value_type, key_compare>::iterator_bst iterator;
		typedef typename BST<value_type, key_compare>::const_iterator_bst const_iterator;
